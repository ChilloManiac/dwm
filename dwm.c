...
static void gettextwithoutcolor(char* in, char* out);
...
buttonpress(XEvent *e)
{
    ...
    char twithoutcolor[1024];
    gettextwithoutcolor(stext, twithoutcolor);
	...
	if (ev->window == selmon->barwin) {
        ...
        //Might need to include getsystray
		else if (ev->x > (x = selmon->ww - TEXTW(twithoutcolor) + lrpad - getsystraywidth())) {
			click = ClkStatusText;
			char textarray[1024];
            gettextwithoutcolor(rawstext, textarray);
            char * text = textarray;
...
void
gettextwithoutcolor(char* in, char* out) {
    int i = -1, j = 0;
    int ignore = 0;
    while(in[++i]) {
        if(in[i] == '^') {
